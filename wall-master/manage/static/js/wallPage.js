...
function submitMsg(id){    
    ...
    let data = {
        ...
        "fontSize":editElement.style.fontSize.replace("px",""),
        "centerRelX":CenterRelX,
        "centerRelY":CenterRelY,
        "width":editElement.style.width.replace("px",""),
        "height":editElement.style.height.replace("px",""),
        "borderRadius":editElement.style.borderRadius.replace("px","")
    };
    
    let xhr = new XMLHttpRequest();
    
    xhr.open("POST",host+"/add",true)
...
function getMessage(){
    showLoading()
    let xhr = new XMLHttpRequest();
    xhr.open("GET",host+"/getMessage",true);
    xhr.send();
    xhr.onreadystatechange = function(){
        completeLoading();
        if(xhr.readyState==4 && xhr.status==200){
            ...
            if(result.code == 200){
                for(let i = 0;i<result.data.length;i++){
                    ...
                    temp_div_userinfo.innerText = "---by---" + result.data[i].username;
...
function signIn() {
    ...
    let xhr = new XMLHttpRequest();
    xhr.open("POST",host+"/signIn",true)
    xhr.setRequestHeader("Content-Type","application/json;charset=utf-8")
    xhr.send(JSON.stringify(data))
    xhr.onreadystatechange = function(){
        if(xhr.readyState==4&&xhr.status==200){
...
window.onload = function(){
    let xhr = new XMLHttpRequest();
    xhr.open("GET",host+"/getUserInfo",true)
    xhr.setRequestHeader("Content-Type","application/json;charset=utf-8")
    xhr.send()
    xhr.onreadystatechange = function(){
        if(xhr.readyState==4&&xhr.status==200){
